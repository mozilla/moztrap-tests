...
class CaseConductorRunTestsPage(CaseConductorBasePage):
    ...
    _test_failed_locator = u'xpath=//section[@id="run"]//article[contains(@class,"item")]//h3[@title="%(case_name)s"]/../div[@class="status"]/span[@class="failed"]'
    ...
    def is_test_failed(self, case_name):
        _test_failed_locator = self._test_failed_locator % {'case_name': case_name}

        return self.is_element_present(_test_failed_locator)
